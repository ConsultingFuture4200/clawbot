# ClawBot — OpenClaw + NemoClaw Local Deployment

## Project Overview

Personal multi-agent AI assistant: OpenClaw inside NemoClaw sandbox on WSL2/Docker.
**Not** a SaaS product, not a custom fork — configuration-only customization of upstream OpenClaw.

## Architecture

- **Host:** Windows + WSL2 (Ubuntu 24.04)
- **GPU:** 2× GTX 1070 Ti (Pascal — local inference limited to ≤7B quantized)
- **Sandbox:** NemoClaw / NVIDIA OpenShell (kernel-level isolation)
- **Gateway:** OpenClaw on port 18789
- **Messaging:** Telegram (primary and only channel)

## Model Strategy (Gemini-Primary)

| Priority | Provider | Model | Role |
|----------|----------|-------|------|
| 1 | Google Gemini | gemini-3-flash | Bulk workhorse (70-80% of requests), free tier |
| 2 | OpenAI Codex | gpt-5.4 (OAuth) | Dev agent only, 5hr/week quota |
| 3 | Anthropic Claude | claude-sonnet-4-6 | Complex reasoning, $30/mo cap |
| 4 | Ollama | qwen2.5:7b | Heartbeats, trivial classification only |
| — | OpenAI API | text-embedding-3-small | Embeddings for memory/search |

## Agents (6 total)

| Agent | Primary Model | Fallback |
|-------|--------------|----------|
| main | gemini-3-flash | codex |
| dev | codex/gpt-5.4 | gemini-3-flash |
| comms | gemini-3-flash | claude-sonnet-4-6 |
| research | gemini-3-flash | (none — 1M context) |
| productivity | gemini-3-flash | codex |
| home | ollama/qwen2.5:7b | gemini-3-flash |
...
clawbot/
├── CLAUDE.md              # This file
├── prd-openclaw-nemoclaw.md
├── .env.example           # API keys template → copy to .env
├── scripts/               # Run 00–09 sequentially in WSL2
├── config/
│   ├── openclaw.json5     # Multi-agent config
│   └── openclaw-sandbox.yaml  # NemoClaw egress policy
├── agents/                # SOUL.md + AGENTS.md per agent
...
2. Run `scripts/00-preflight.sh` through `scripts/09-verify-phase1.sh` sequentially in WSL2
3. Script 09 validates all 14 Phase 1 exit criteria
...
- **Home agent confirms destructive actions** — locks, alarms, cameras

## Anti-Patterns (from PRD §12)

- Claude subscription OAuth tokens in OpenClaw (ToS violation)
- Google AI Ultra OAuth (account restrictions)
- OpenClaw outside NemoClaw sandbox
- Agents modifying own config/auth
- Auto-sending emails
- Hardcoding API keys in openclaw.json
- Docker Desktop on Windows
- Nemotron/large models on GTX 1070 Ti
- Codex for non-code tasks
- Heartbeats on cloud providers
